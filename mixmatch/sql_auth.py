...
def get_projects_at_sp(service_provider, user_token):
    local_auth = get_local_auth(user_token)
    projects = model.SpProject.find(local_user_id=local_auth.get_user_id(),
                                    sp=service_provider)
    return [project.remote_project_id for project in projects]
...
def get_sp_auth(service_provider, user_token, remote_project_id):
    ...
    local_auth = get_local_auth(user_token)
    local_user_id = local_auth.get_user_id()
    sp_auth = model.SpAuth.find(local_user_id, service_provider)

    remote_auth = identity.Password(
        auth_url=config.get_conf_for_sp(service_provider).auth_url,
        username=sp_auth.remote_username,
        user_domain_id=sp_auth.remote_user_domain_id,
        password=sp_auth.remote_password,
